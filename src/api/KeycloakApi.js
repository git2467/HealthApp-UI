...
import { fetchFoodDiaryById } from "../api/EngineApi"; //aljw
...
export const setTokenInAxios = (token) => {
  if (token) {
    engineAxiosInstance.defaults.headers.common['Authorization'] = `Bearer ${token}`;
  } else {
...
export const refreshToken = async () => {
  try {
    ...
    if (!refreshToken) {
      console.error("No refresh token found while attempting to refresh token.");
      return;
    }
